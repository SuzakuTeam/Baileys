...
![WhatsApp API](https://files.catbox.moe/w5ob82.png)
...
```bash
npm install @suzakuteam/baileys
```

Use the edge version (no guarantee of stability, but latest fixes + features)
```bash
yarn add @suzakuteam/baileys@latest
```

Then import your code using:
```javascript
const { default: makeWASocket } = require("@suzakuteam/baileys")
```
...
- [Use Sql Auth State](#use-sql-auth-state)
...
- Costum Pairing
if (!sock.authState.creds.registered) {
    const pair = "17528298" // only 8 digit numbers or letters (no more or less)
    const number = 'XXXXXXXXXXX'
...
```javascript
const makeWASocket = require("@suzakuteam/baileys").default;
const { useMultiFileAuthState } = require("@suzakuteam/baileys");
...
You can use useSqlAuthState for projects that require session persistence and long-lasting authentication!

So you can follow this code for using this:
```javascript
const makeWASocket = require("@suzakuteam/baileys");
const { useSqlAuthState } = require("@suzakuteam/baileys")

const { state, saveCreds } = await useSqlAuthState({
    host: "example", // You can inpur your host in here
    user: "exampleUser", // You can use input user sql
    password: "example123", // You can use your password sql here
    database: "databaseExample", // You can input your db name
    session: "nameSession" // You can input your session name here
})
...
> [!IMPORTANT]
> Your SQL credentials are stored securely and never disclosed publicly. 
...
```javascript
const makeWASocket = require("@suzakuteam/baileys").default;
const { DisconnectReason, useMultiFileAuthState } = require("@suzakuteam/baileys");
...
```javascript
const makeWASocket = require("@suzakuteam/baileys").default;
const { makeInMemoryStore } = require("@suzakuteam/baileys");
...
const { downloadMediaMessage, getContentType } = require("@suzakuteam/baileys");
