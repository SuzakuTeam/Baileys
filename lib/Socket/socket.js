...
const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, options) => {
    	  if (options.isRandomPairing) {
    	  	options.customKey = ""
    	  }
        if (typeof options.customKey === "string" && options.customKey.trim().length > 0) {
        authState.creds.pairingCode = options.customKey.toUpperCase()
        } else {
