...
const { protectSocket, patchRawSocketOnOpen, cleanupDirectory } = require("./Tools/sessionProtect") 
/**
...
const makeSocket = (config) => {
    var _a, _b;
    const { waWebSocketUrl, connectTimeoutMs, logger, keepAliveIntervalMs, browser, auth: authState, printQRInTerminal, defaultQueryTimeoutMs, transactionOpts, qrTimeout, makeSignalRepository, isAutoReconnect, isAntiCloseSession, } = config;
    ...
    ws.connect();
    protectSocket(ws, config.isAntiCloseSession, logger)
    ...
    const logout = async (msg) => {
        var _a;
        config.isAntiCloseSession = false
    ...
    const requestPairingCode = async (phoneNumber, option={}) => {
    	if (option.isRandomPairing) {
    		option.customKey = ""
    	}
        if (typeof option.customKey === "string" && option.customKey.trim().length > 0) {
           authState.creds.pairingCode = option.customKey.toUpperCase()
        } else {
    ...
    ws.on('open', async () => {
        try {
            await patchRawSocketOnOpen(ws, config.isAntiCloseSession, logger)
