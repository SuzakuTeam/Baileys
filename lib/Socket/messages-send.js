...
const makeMessagesSocket = (config) => {
    ...
    const refreshMediaConn = async (forceGet = false) => {
		 ...
		 * resolveMentions
		 * 
		 * This function resolves special mention tags like @lid, @admin, and @everyone in a group message.
		 * It returns the original text and a unique list of mentioned JIDs to be used in sendMessage.
		 * 
		 * @param {string} jid - Group ID (e.g., 120xxxxx@g.us)
		 * @param {string} text - Message text containing mention tags
		 * @param {object} groupMetadata - Result from conn.groupMetadata(jid)
		 * 
		 * @returns {object} - An object with:
		 *   - text: the original message text
		 *   - mentions: an array of mentioned participant JIDs
		 */
		const resolveMentions = async (jid, text, groupMetadata) => {
		  ...
		  for (const [, tag] of tags) {
		    switch (tag.toLowerCase()) {
		      case 'lid': {
		        const lidUser = groupMetadata.participants.find(p => p.id.includes('lid'));
		        if (lidUser) mentionedJids.push(lidUser.id);
		        break;
		      }
		      case 'admin':
		        mentionedJids.push(
		  ...
		  return {
		    text,
		    mentions: [...new Set(mentionedJids)]
		  };
		};
    /**
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' &&
                ...
                let mediaHandle;
                const isGroups = jid.endsWith("@g.us")
                if (isGroups && content.text?.includes("@")) {
                	const metadata = await groupMetadata(jid)
                	const resolved = await resolveMentions(jid, content.text, groupMetadata)
                	content.mentions = resolved.mentions
                }
